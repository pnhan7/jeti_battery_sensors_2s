...
void JETI_Box_class::setValueBig(uint8_t ident, unsigned int* valuebig) {
	valueEX[ident]=(unsigned int*)valuebig;
	precisionEX[ident]=4;
}
	...
	if (!alarmEX ) {
	   ...
	   frame[6]=JETI_SENSOR_ID4;
		...
		if (sendheader == 0) {
			...
			for (i_Loop=0; i_Loop<maxvals; i_Loop++) {
				...
				if (precisionEX[identSend]<4) { 
					...
					cmpt++;
				} else if (precisionEX[identSend]==4) {
					frame[cmpt]|=0x04;
					cmpt++;
					valeur2=(uint16_t*)valueEX[identSend];
					frame[cmpt]=valeur2[0]&0xff;
					cmpt++;
					frame[cmpt]=valeur2[0]>>8;
					cmpt++;
					frame[cmpt]=valeur2[0];
					frame[cmpt]&=0x00;
					cmpt++;
				} else if (precisionEX[identSend]<7) { // WAS less than 6
					frame[cmpt]|=0x05;
					cmpt++;
